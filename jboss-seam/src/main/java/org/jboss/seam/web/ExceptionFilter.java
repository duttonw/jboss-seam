...
import org.jboss.seam.mock.DummyExternalContext;
...
public class ExceptionFilter extends AbstractFilter {
	...
	protected void endWebRequestAfterException(HttpServletRequest request, HttpServletResponse response, Exception e)
		...
		DummyExternalContext dummyExternalContext = new DummyExternalContext(getServletContext(), request, response);

		//Initialize the temporary context objects
		FacesLifecycle.beginExceptionRecovery(dummyExternalContext);
		...
		try {
			...
			try {
				FacesLifecycle.endRequest(dummyExternalContext);
